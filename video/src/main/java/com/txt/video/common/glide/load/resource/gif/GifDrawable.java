...
import static com.txt.video.common.glide.load.resource.gif.GifDecoder.TOTAL_ITERATION_COUNT_FOREVER;


/**
 * An animated {@link Drawable} that plays the frames of an animated GIF.
 */
public class GifDrawable extends GlideDrawable implements com.txt.video.common.glide.load.resource.gif.GifFrameLoader.FrameCallback {
    private final Paint paint;
    private final Rect destRect = new Rect();
    private final GifState state;
    private final GifDecoder decoder;
    private final com.txt.video.common.glide.load.resource.gif.GifFrameLoader frameLoader;
     ...
     *
     * @see #setFrameTransformation(com.txt.video.common.glide.load.Transformation, Bitmap)
     *
     * @param context A context.
     * @param bitmapProvider An {@link com.txt.video.common.glide.gifdecoder.GifDecoder.BitmapProvider} that can be used to
     *                       retrieve re-usable {@link Bitmap}s.
     * @param bitmapPool A {@link com.txt.video.common.glide.load.engine.bitmap_recycle.BitmapPool} that can be used to return
     *                   the first frame when this drawable is recycled.
     * @param frameTransformation An {@link com.txt.video.common.glide.load.Transformation} that can be applied to each frame.
     * @param targetFrameWidth The desired width of the frames displayed by this drawable (the width of the view or
     *                         {@link com.txt.video.common.glide.request.target.Target} this drawable is being loaded into).
    ...
    public GifDrawable(com.txt.video.common.glide.load.resource.gif.GifDrawable other, Bitmap firstFrame,
    ...
    GifDrawable(GifState state) {
        ...
        frameLoader = new com.txt.video.common.glide.load.resource.gif.GifFrameLoader(state.context, this, decoder, state.targetWidth, state.targetHeight);
        frameLoader.setFrameTransformation(state.frameTransformation);
    }

    // Visible for testing.
    GifDrawable(GifDecoder decoder, com.txt.video.common.glide.load.resource.gif.GifFrameLoader frameLoader, Bitmap firstFrame, BitmapPool bitmapPool, Paint paint) {
    ...
    static class GifState extends Drawable.ConstantState {
        ...
        public Drawable newDrawable() {
            return new com.txt.video.common.glide.load.resource.gif.GifDrawable(this);
        }
