...
import com.txt.video.common.glide.request.ResourceCallback;
 ...
 * load completes.
 */
class EngineJob implements com.txt.video.common.glide.load.engine.EngineRunnable.EngineRunnableManager {
    ...
    private final com.txt.video.common.glide.load.engine.EngineJobListener listener;
    ...
    private com.txt.video.common.glide.load.engine.EngineRunnable engineRunnable;
    private com.txt.video.common.glide.load.engine.EngineResource<?> engineResource;

    private volatile Future<?> future;

    public EngineJob(Key key, ExecutorService diskCacheService, ExecutorService sourceService, boolean isCacheable,
                     com.txt.video.common.glide.load.engine.EngineJobListener listener) {
        this(key, diskCacheService, sourceService, isCacheable, listener, DEFAULT_FACTORY);
    }

    public EngineJob(Key key, ExecutorService diskCacheService, ExecutorService sourceService, boolean isCacheable,
                     com.txt.video.common.glide.load.engine.EngineJobListener listener, EngineResourceFactory engineResourceFactory) {
        this.key = key;
    ...
    public void start(com.txt.video.common.glide.load.engine.EngineRunnable engineRunnable) {
        this.engineRunnable = engineRunnable;
        future = diskCacheService.submit(engineRunnable);
    }

    @Override
    public void submitForSource(com.txt.video.common.glide.load.engine.EngineRunnable runnable) {
    ...
    static class EngineResourceFactory {
        public <R> com.txt.video.common.glide.load.engine.EngineResource<R> build(Resource<R> resource, boolean isMemoryCacheable) {
            return new com.txt.video.common.glide.load.engine.EngineResource<R>(resource, isMemoryCacheable);
        }
    }
    ...
    private static class MainThreadCallback implements Handler.Callback {
        ...
        public boolean handleMessage(Message message) {
            if (MSG_COMPLETE == message.what || MSG_EXCEPTION == message.what) {
                com.txt.video.common.glide.load.engine.EngineJob job = (com.txt.video.common.glide.load.engine.EngineJob) message.obj;
