 ...
 * remote {@link Uri}s to a wrapped {@link com.txt.video.common.glide.load.model.ModelLoader} that handles
 * {@link com.txt.video.common.glide.load.model.GlideUrl}s.
 *
...
public abstract class UriLoader<T> implements ModelLoader<Uri, T> {
    ...
    public final DataFetcher<T> getResourceFetcher(Uri model, int width, int height) {
        ...
        if (isLocalUri(scheme)) {
            if (com.txt.video.common.glide.load.model.AssetUriParser.isAssetUri(model)) {
                String path = com.txt.video.common.glide.load.model.AssetUriParser.toAssetPath(model);
