...
public class MeetingPageMultiChatLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    private static final String TAG = MeetingPageMultiChatLayoutManager.class.getName();
     ...
     */
    public MeetingPageMultiChatLayoutManager(@IntRange(from = 1, to = 100) int rows,
...
//            addView(scrap);
//            measureChildWithMargins(scrap, mItemWidth, getUsableHeight() - mItemWidth);
...
//            //测量view的大小，这个是控制view大小的关键
//            addView(scrap);
//            measureChildWithMargins(scrap, mItemWidth, getHeight() - mItemWidth);
//            //把view放到哪个位置，长宽要跟view的大小匹配，不然view看不到了
//            layoutDecorated(scrap,
//                    mItemWidth,
//                    0,
//                    mItemWidth * 2,
...
//        }
        ...
        if (canScrollHorizontally()) {
            ...
            mMaxScrollX = 0;
            mMaxScrollY = (mPageCount - 1) * getUsableWidth();
...
//        onLayoutChildren(recycler, state);
        ...
        if (null == rect) {
            ...
            if (mColumns == 3) {
                ...
                if (row >= 1) {
                    ...
                    } else if (getItemCount() % mColumns == 2 && allSize == getItemCount()) {

                        offsetX = offsetX + col * mItemWidth + mItemWidth / 2;
                        offsetY = offsetY + row * mItemWidth;
                    }else if (getItemCount() % mColumns == 2 && allSize == getItemCount()-1){
                        offsetX = offsetX + mItemWidth / 2;
                        offsetY = offsetY + row * mItemWidth;
                    } else {
                        offsetX += col * mItemWidth;
                        offsetY += row * mItemWidth;
                    }
                } else {
                    offsetX += col * mItemWidth;
                    offsetY += row * mItemWidth;
            ...
            rect.bottom = offsetY + mItemWidth;
    ...
    public void scrollToPage(int pageIndex) {
        ...
        if (canScrollVertically()) {
            mTargetOffsetXBy = 0;
            mTargetOffsetYBy = pageIndex * getUsableWidth() - mOffsetY;
        } else {
    ...
    public interface PageListener {
        ...
        void onLayoutCompleted(int fromItem);
    }
