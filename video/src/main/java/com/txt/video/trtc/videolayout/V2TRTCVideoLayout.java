 ...
 * 作用：
 * 1. 实现了手势监听，配合 {@link V2TRTCVideoLayoutManager} 能够实现自由拖动 View。
 * 详情可见：{@link V2TRTCVideoLayout#initGestureListener()}
 * 实现原理：利用 RelativeLayout 的 margin 实现了能够在父容器自由定位的特性；需要注意，{@link V2TRTCVideoLayout} 不能增加约束规则，如 alignParentRight 等，否则无法自由定位。
 * <p>
 * 2. 对{@link TXCloudVideoView} 与逻辑 UI 进行组合，在 muteLocal、音量回调等情况，能够进行 UI 相关的变化。若您的项目中，也相关的业务逻辑，可以参照 Demo 的相关实现。
 */
public class V2TRTCVideoLayout extends RelativeLayout implements View.OnClickListener {
    ...
    public V2TRTCVideoLayout(Context context) {
        this(context, null);
    }

    public V2TRTCVideoLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public V2TRTCVideoLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setAudioVolumeProgress(int progress) {
        if (mPbAudioVolume != null) {
            ...
            if (progress == -1) {
                ...
                mPbAudioVolume.setImageResource(R.drawable.tx_icon_volume_small_0);
        ...
        mSimpleOnGestureListener = new GestureDetector(getContext(), new GestureDetector.SimpleOnGestureListener() {
            ...
            public boolean onSingleTapUp(MotionEvent e) {
                if (mClickListener != null) {
                    mClickListener.onClick(V2TRTCVideoLayout.this);
                }
                return true;
            }
            ...
            public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
                ...
                ViewGroup.LayoutParams params = V2TRTCVideoLayout.this.getLayoutParams();
                // 当 TRTCVideoView 的父容器是 RelativeLayout 的时候，可以实现拖动
                if (params instanceof LayoutParams) {
                    LayoutParams layoutParams = (LayoutParams) V2TRTCVideoLayout.this.getLayoutParams();
                    ...
                    V2TRTCVideoLayout.this.setLayoutParams(layoutParams);
                }
                return true;
            }
        });
    ...
    public interface IVideoLayoutListener {
        void onClickFill(V2TRTCVideoLayout view, boolean enableFill);

        void onClickMuteAudio(V2TRTCVideoLayout view, boolean isMute);

        void onClickMuteVideo(V2TRTCVideoLayout view, boolean isMute);

        void onClickMuteInSpeakerAudio(V2TRTCVideoLayout view, boolean isMute);
    }
}
